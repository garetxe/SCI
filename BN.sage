...
# Compute the flavor representation of the lowest R-charge B-baryon in
# the SU(N) theory. Generally we truncate the result to non-vanishing
# reps of SU(3), but a full result (without taking into account this
# vanishing) can be requested by setting full_result=True.
def BN(N, full_result=False):
    partitions = partitions_list(N)
    if not full_result:
        # Partitions with more than 3 rows vanish on the flavor side.
        partitions = [part for part in partitions if len(part) < 4]
    ...
    for part in partitions:
        ...
        n = n_singlets(N-1, LiE_plethysm(part, LiE))
...
# Keep computing all BN, until interrupted, unless a final value for N
# (wich will also be computed) is given. Generally we truncate the
# result to non-vanishing reps of SU(3), but a full result (without
# taking into account this vanishing) can be requested by setting
# full_result=True.
def all_BN(starting_N=3, final_N=None, full_result=False):
    N = starting_N-2
    while True:
        N += 2
        if final_N and N > final_N:
            break
        if not full_result:
            fname = "B{0}.result".format(N)
        else:
            fname = "B{0}.full.result".format(N)
        # Check if this was already computed, skip if that's the case.
        if os.path.exists(fname):
            continue
        rep = BN(N, full_result)
	fd = open(fname, "w")
        fd.write(str(rep)+"\n")
        fd.close()        
